...
const LEAGUES = [
  { abv: 'NBA', name: 'NBA', count: '5 matchs analysés' },
  { abv: 'WNBA', name: 'WNBA', count: '3 matchs analysés' },
  { abv: 'EL', name: 'EuroLeague', count: '2 matchs analysés' },
  { abv: 'NCAA', name: 'NCAA', count: 'Couverture en cours' },
];
...
export default function HomePage() {
  ...
  return (
    <main className="flex-1">
      ...
      {/* ===== BANDEAU LIGUES ===== */}
      <section className="border-b border-surface-line bg-court py-9">
        <p className="mb-5 text-center font-display text-[11px] uppercase tracking-[2px] text-bone-dim">
          Ligues couvertes par l&apos;analyse HOOPIUM
        </p>
        <div className="flex flex-wrap justify-center gap-10 px-6">
          {LEAGUES.map((league) => (
            <div key={league.abv} className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-[10px] border border-surface-line bg-surface font-display text-xs font-bold text-orange">
                {league.abv}
              </div>
              <div>
