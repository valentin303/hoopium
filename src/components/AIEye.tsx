...
export function AIEye({ text, variablesUsed, active }: { text: string; variablesUsed: number; active: boolean }) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl border border-orange-dim p-7"
      style={{ background: 'linear-gradient(160deg, rgba(255,107,26,.06), transparent 40%), #121212' }}
    >
      ...
      <p className="min-h-[96px] text-base leading-relaxed text-bone">
        {shown}
        {!done && active && <span className="ml-0.5 inline-block h-[1.1em] w-[2px] animate-pulse bg-orange align-[-2px]" />}
      </p>
